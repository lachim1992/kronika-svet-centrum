...
Deno.serve(async req => {
  ...
  try {
    ...
    const r=await sb.from("economy_turn_ledgers").select("turn_number").eq("session_id",session_id).eq("turn_number",s.data.current_turn).single();
    if(r.error)throw Error("Missing mandatory goods projection");
