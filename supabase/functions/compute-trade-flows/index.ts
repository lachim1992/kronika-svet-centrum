...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [goodsRes, recipesRes, nodesRes, citiesRes, routesRes, hexesRes, ordersRes, roadRes, riverRes] = await Promise.all([
      ...
      sb.from("province_nodes").select("id, session_id, node_type, node_tier, node_subtype, production_role, capability_tags, guild_level, city_id, controlled_by, production_output, hex_q, hex_r, grid_x, grid_y, infrastructure_level, upgrade_level, specialization_scores, parent_node_id, route_access_factor, trade_system_id").eq("session_id", session_id),
    ...
    for (const g of goods) {
      goodToBasket.set(g.key, resolveBasketKey(g.demand_basket || "staple_food", warnings));
    }

    // Index orders by node_id
    const orderByNode = new Map<string, { basket: string; mode: string; good?: string | null }>();
    for (const o of (ordersRes.data as any[]) || []) {
      orderByNode.set(o.node_id, {
        basket: resolveBasketKey(o.target_basket_key, warnings),
        mode: o.mode,
        good: o.target_good_key,
      });
    }
    ...
    // ════════════════════════════════════════════
    // PHASE 3.5: Flow centrality (Trade Hubs) — actual routed traffic through node cells
    // ════════════════════════════════════════════
    try {
      const nodesByCell = new Map<string, string[]>();
      for (const n of nodes) {
        const key = `${n.hex_q},${n.hex_r}`;
        nodesByCell.set(key, [...(nodesByCell.get(key) || []), n.id]);
      }
      const rawCentrality = new Map<string, number>();
      let maxTraffic = 0;
      for (const f of tradeFlows) {
        const volume = Math.max(0, Number((f as any).volume_per_turn || 0));
        const cells = Array.isArray((f as any).path_cells) ? (f as any).path_cells : [];
        for (const cell of cells.slice(1, -1)) {
          for (const nodeId of nodesByCell.get(`${cell.x},${cell.y}`) || []) {
            const next = (rawCentrality.get(nodeId) || 0) + volume;
            rawCentrality.set(nodeId, next); maxTraffic = Math.max(maxTraffic, next);
          }
        }
      }
      await sb.from("province_nodes").update({ flow_centrality: 0 }).eq("session_id", session_id);
      for (const [nodeId, traffic] of rawCentrality) {
        const val = maxTraffic > 0 ? traffic / maxTraffic : 0;
