...
- [ ] **Etapa 7 — `process-turn` construction tick**: progress = soldiers × engineering × terrain × risk_mult; ambush check; complete → lifecycle='usable', stack=idle. Upkeep + over-mobilization penalty (×1.5 upkeep, ×0.85 city prod, +unrest).
- [ ] **Etapa 8 — WorldMap UI**: Trade Systems overlay, Build Mode (A* preview + risk badges), Routes overlay (under_construction dashed pulsing), NeutralNodePanel rozšíření, HUD Manpower karta, Army Panel assignment.
