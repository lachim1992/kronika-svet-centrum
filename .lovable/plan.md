...
- **domestic_tax_base** = hodnota skutečně uspokojené domácí spotřeby:
  `goods_domestic_consumption_value = Σ (local_demand − unmet_demand) × basketValue`
  z post-trade `city_market_baskets`. Import se nepočítá znovu a spotřeba se
  nerekonstruuje z `local_supply` (to je mutable pole, do kterého solver foldí import).
- **extraction_tax_base** = hodnota realizovaného outputu extractive sektoru.
  `goods_extraction_value` se počítá **už při produkci receptu** podle `production_role`
  původního uzlu (`source`); provenience se nesmí zpětně odvozovat z výsledného koše
  (stejný koš může vzniknout různými výrobními cestami).
...
- **famine / food balance**, `last_turn_grain_prod` / `_cons` / `_net` a **faction food
  ...
  `cityEcon.balance`.
- **production_reserve** — vědomě breaking změna: akumulace z `totalCityProduction` se ruší
  a nedostane ad hoc nový vzorec. **Před release ověřím, jestli je production_reserve
  jediný obnovitelný CAPEX zdroj pro stavby a silnice.** Pokud ano, dočasná zásoba bez
  přírůstku by se vyčerpala a stavby by přestaly být možné — pak musí navazující
  construction-goods → CAPEX pass přijít ještě před hraním, ne později. Zjištění reportuji,
  žádnou náhradní konverzi si v tomto passu nevymýšlím.
